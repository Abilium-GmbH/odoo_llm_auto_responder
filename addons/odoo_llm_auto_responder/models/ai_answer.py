...
class HelpdeskTicket(models.Model):
    ...
    # Version 1
        """self.env.cr.execute("insert into helpdesk.ticket (ai_answer) values ('Das ist eine vorgefertigte Antwort!')")
        self.env.cr.commit()

        res = self.env.cr.fetchall("select description from helpdesk.ticket")
        logger.info(res)
